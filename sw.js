// Service worker: lets the site open offline and be installed as an app.
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys()
      .then((keys) => Promise.all(keys.filter((k) => k.startsWith('cip-') && k !== CACHE).map((k) => caches.delete(k))))
